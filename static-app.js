...
const RECENT_KEY = 'rehome-recent-products-v1';
const REVIEW_KEY = 'rehome-product-reviews-v1';
...
const readCart = () => { try { return (JSON.parse(localStorage.getItem(CART_KEY)) || []).map(item => ({...item, option:item.option || '기본 옵션', lineId:item.lineId || `${item.id}::${item.option || '기본 옵션'}`})); } catch { return []; } };
...
const rememberProduct = (id) => { const recent = [id, ...readRecent().filter(itemId => itemId !== id)].slice(0, 3); localStorage.setItem(RECENT_KEY, JSON.stringify(recent)); };
const readReviews = () => { try { return JSON.parse(localStorage.getItem(REVIEW_KEY)) || {}; } catch { return {}; } };
const writeReviews = (reviews) => localStorage.setItem(REVIEW_KEY, JSON.stringify(reviews));
...
const syncNavigationPosition = () => {
  const navigation = document.querySelector('.static-nav');
  if (!navigation) return;
  if (matchMedia('(min-width: 901px)').matches) navigation.style.top = `${Math.max(0, 80 - window.scrollY)}px`;
  else navigation.style.top = '0px';
};
let navigationFrame;
window.addEventListener('scroll', () => { cancelAnimationFrame(navigationFrame); navigationFrame = requestAnimationFrame(syncNavigationPosition); }, { passive: true });
window.addEventListener('resize', syncNavigationPosition);
syncNavigationPosition();

const renderUtilityRail = () => {
  const main = document.querySelector('main');
  if (!main) return;
  const recentProducts = readRecent().map(id => products.find(product => product.id === id)).filter(Boolean);
  const rail = document.createElement('aside');
  rail.className = 'rehome-utility-rail';
  rail.setAttribute('aria-label', '페이지 도구');
  rail.innerHTML = `<div class="recent-products"><h2>최근 본 상품</h2>${recentProducts.length ? `<ul>${recentProducts.map(product => `<li><a href="${href(`detail.html?id=${product.id}`)}"><img src="${image(product.image)}" alt="${product.name}"><span>${product.name}</span></a></li>`).join('')}</ul>` : '<p>최근 본 상품이 없습니다.</p>'}</div><div class="page-jumps"><button type="button" data-page-up aria-label="페이지 맨 위로">↑</button><button type="button" data-page-down aria-label="페이지 맨 아래로">↓</button></div>`;
  main.appendChild(rail);
  rail.querySelector('[data-page-up]').addEventListener('click', () => window.scrollTo({ top: 0, behavior: 'smooth' }));
  rail.querySelector('[data-page-down]').addEventListener('click', () => window.scrollTo({ top: document.documentElement.scrollHeight, behavior: 'smooth' }));
};
...
const renderHome = () => {
  ...
  let sliderTimer; const startSlider = () => { clearInterval(sliderTimer); sliderTimer = setInterval(() => show(current + 1), 4500); }; const stopSlider = () => clearInterval(sliderTimer);
  document.querySelector('.next').addEventListener('click', e => { e.preventDefault(); show(current + 1); startSlider(); });
  document.querySelector('.prev').addEventListener('click', e => { e.preventDefault(); show(current - 1); startSlider(); });
  document.querySelector('#slide_wrap').addEventListener('mouseenter', stopSlider); document.querySelector('#slide_wrap').addEventListener('mouseleave', startSlider);
  document.addEventListener('visibilitychange', () => document.hidden ? stopSlider() : startSlider());
  startSlider();
};

const renderProducts = () => {
  const params = new URLSearchParams(location.search); const query = (params.get('search') || '').trim(); const category = params.get('category') || 'table';
  document.body.dataset.category = category;
  document.body.classList.toggle('is-search', Boolean(query));
  const title = query ? `SEARCH · ${query}` : category.toUpperCase();
  const results = query ? products.filter(item => `${item.name} ${item.subtitle} ${item.category}`.toLowerCase().includes(query.toLowerCase())) : products.filter(item => item.category === category);
  document.querySelectorAll('[data-category-title]').forEach(node => node.textContent = title);
  document.querySelector('[data-category-products]').innerHTML = results.length ? results.map(item => card(item)).join('') : `<li class="search-empty"><strong>검색 결과가 없습니다.</strong><p>다른 검색어를 입력하거나 전체 상품을 둘러보세요.</p><a href="${href('products.html?category=table')}">전체 상품 보기</a></li>`;
};
...
const renderDetail = () => {
  ...
  document.querySelector('[data-product-detail]').innerHTML = `<table id="itemDetail"><tbody><tr><td><img class="detail-static-image" src="${image(detailExists)}" alt="${product.name}"></td><td><div id="detailText"><h3>${product.name}</h3><p>${product.subtitle}</p></div><div id="detailPrice"><ul><li>판매가</li><li>배송비</li><li>적립금</li><li>총 상품금액</li></ul><ul><li id="productPrice">${product.price.toLocaleString('ko-KR')}원</li><li>무료배송</li><li>${Math.round(product.price * .01).toLocaleString('ko-KR')}원</li><li class="detail-total" data-detail-total>${product.price.toLocaleString('ko-KR')}원</li></ul></div><div id="detailSelect"><label for="detail-option">옵션</label><select id="detail-option"><option>기본 옵션</option><option>내추럴 오크</option><option>월넛</option></select></div><div class="quantity-row"><button type="button" data-qty-minus aria-label="수량 줄이기">−</button><input data-qty type="number" min="1" value="1" aria-label="상품 수량"><button type="button" data-qty-plus aria-label="수량 늘리기">+</button></div><div id="buttons"><button id="cartBtn" type="button" data-add-cart>장바구니</button><button id="buyBtn" type="button" data-buy>구매하기</button></div></td></tr></tbody></table><div id="detailProduct"><nav class="detail-anchor-nav" aria-label="상품 상세 메뉴"><a href="#details">상품정보</a><a href="#delivery-information">배송정보</a><a href="#exchange-information">교환/반품</a><a href="#review-contents">상품후기</a></nav><section id="details" class="detail-section"><h3>PRODUCT DETAIL</h3><p>일상에 편안함을 더하는 ReHome의 가구 컬렉션입니다.</p><img src="${image(detailExists)}" alt="${product.name} 상세 이미지"></section><section id="delivery-information" class="detail-section"><h3>배송정보 및 유의사항</h3><p>DELIVERY INFORMATION & CAUTION</p><table class="detail-info-table"><tbody><tr><th>배송기간</th><td>주문 확인 후 평균 3–7일 이내 배송됩니다. 주문 제작 상품과 지역에 따라 일정이 달라질 수 있습니다.</td></tr><tr><th>일정 변경</th><td>배송일 변경은 예정일 최소 2일 전 고객센터로 접수해 주세요.</td></tr><tr><th>유의사항</th><td>엘리베이터 사용료, 사다리차 및 현장 상황에 따른 추가 비용은 별도로 안내됩니다.</td></tr></tbody></table></section><section id="exchange-information" class="detail-section"><h3>교환 및 반품 안내</h3><p>상품 수령 후 7일 이내 고객센터를 통해 접수해 주세요. 설치 또는 사용 흔적이 있는 제품, 주문 제작 제품은 단순 변심으로 교환·반품이 어려울 수 있습니다.</p></section><section id="review-contents" class="detail-section review-section"><h3>상품후기</h3><form class="review-form" data-review-form><label>후기 작성<textarea name="review" rows="5" required placeholder="상품후기를 작성해 주세요."></textarea></label><div><label>평점<select name="rating"><option value="5">★★★★★</option><option value="4">★★★★</option><option value="3">★★★</option><option value="2">★★</option><option value="1">★</option></select></label><label class="review-file">사진 선택<input type="file" name="photo" accept="image/*"><span data-review-file-name>선택된 파일 없음</span></label><button type="submit">후기 등록</button></div></form><div class="review-list" data-review-list></div></section></div>`;
  const qty = document.querySelector('[data-qty]');
  const totalNode = document.querySelector('[data-detail-total]'); const updateDetailTotal = () => { qty.value = Math.max(1, Number(qty.value) || 1); totalNode.textContent = `${(product.price * Number(qty.value)).toLocaleString('ko-KR')}원`; };
  document.querySelector('[data-qty-minus]').onclick = () => { qty.value = Math.max(1, Number(qty.value) - 1); updateDetailTotal(); };
  document.querySelector('[data-qty-plus]').onclick = () => { qty.value = Number(qty.value) + 1; updateDetailTotal(); };
  qty.addEventListener('change', updateDetailTotal);
  const add = (goCart) => { const option = document.querySelector('#detail-option').value; const lineId = `${product.id}::${option}`; const cart = readCart(); const found = cart.find(item => item.lineId === lineId); if(found) found.quantity += Number(qty.value); else cart.push({...product, option, lineId, quantity:Number(qty.value)}); writeCart(cart); toast(`${option} 옵션을 장바구니에 담았습니다.`); if(goCart) location.href = href('cart.html'); };
  document.querySelector('[data-add-cart]').onclick = () => add(false); document.querySelector('[data-buy]').onclick = () => add(true);
  const reviewList = document.querySelector('[data-review-list]');
  const renderReviewList = () => { const reviews = readReviews()[product.id] || []; reviewList.innerHTML = reviews.length ? reviews.map(review => `<article><div><strong>${'★'.repeat(review.rating)}${'☆'.repeat(5-review.rating)}</strong><time>${review.date}</time></div><p>${review.text}</p>${review.photo ? `<small>첨부: ${review.photo}</small>` : ''}<button type="button" data-review-delete="${review.id}" aria-label="후기 삭제">×</button></article>`).join('') : '<p class="empty-reviews">아직 등록된 상품후기가 없습니다.</p>'; };
  document.querySelector('[name="photo"]').addEventListener('change', event => { document.querySelector('[data-review-file-name]').textContent = event.target.files[0]?.name || '선택된 파일 없음'; });
  document.querySelector('[data-review-form]').addEventListener('submit', event => { event.preventDefault(); const data = new FormData(event.target); const allReviews = readReviews(); const productReviews = allReviews[product.id] || []; productReviews.unshift({ id: Date.now(), text:String(data.get('review')).trim(), rating:Number(data.get('rating')), photo:event.target.elements.photo.files[0]?.name || '', date:new Date().toLocaleDateString('ko-KR') }); allReviews[product.id] = productReviews; writeReviews(allReviews); event.target.reset(); document.querySelector('[data-review-file-name]').textContent = '선택된 파일 없음'; renderReviewList(); toast('상품후기가 등록되었습니다.'); });
  reviewList.addEventListener('click', event => { const button = event.target.closest('[data-review-delete]'); if(!button) return; const allReviews = readReviews(); allReviews[product.id] = (allReviews[product.id] || []).filter(review => String(review.id) !== button.dataset.reviewDelete); writeReviews(allReviews); renderReviewList(); });
  renderReviewList();
};
...
document.querySelectorAll('[data-search-form]').forEach(form => form.addEventListener('submit', event => { event.preventDefault(); const query = String(new FormData(form).get('searchValue') || '').trim(); if(!query){ toast('검색어를 입력해 주세요.'); form.querySelector('input')?.focus(); return; } location.href = href(`products.html?search=${encodeURIComponent(query)}`); }));
...
const renderInterior = () => {
  ...
  const hotspotPositions = [[18,68],[53,62],[78,70]];
  root.innerHTML = `<div class="subpage-inner interior-static"><h1 class="subpage-title">INTERIOR DESIGN</h1>${tabs}<header class="interior-intro"><span>REHOME SPACE</span><h2>${data.title}</h2><p>${data.intro}</p><small>사진 속 표시를 선택하면 해당 제품을 볼 수 있습니다.</small></header><div class="interior-scenes">${data.images.map((file, index) => { const product = data.products[index]; const sceneProducts = [data.products[index], data.products[(index+1)%data.products.length], data.products[(index+2)%data.products.length]]; return `<article class="interior-scene"><div class="scene-image"><img src="${image(file)}" alt="${data.title} 인테리어 ${index + 1}"><span>${String(index + 1).padStart(2,'0')}</span><div class="scene-hotspots">${sceneProducts.map((sceneProduct, hotspotIndex) => `<a class="scene-hotspot" style="--hotspot-x:${hotspotPositions[hotspotIndex][0]}%;--hotspot-y:${hotspotPositions[hotspotIndex][1]}%" href="${href(`detail.html?id=${sceneProduct.id}`)}" aria-label="${sceneProduct.name} 상세 보기"><i aria-hidden="true">+</i><b><strong>${sceneProduct.name}</strong><em>${sceneProduct.price.toLocaleString('ko-KR')}원</em></b></a>`).join('')}</div></div><a class="scene-product" href="${href(`detail.html?id=${product.id}`)}"><img src="${image(product.image)}" alt="${product.name}"><span><small>SHOP THE LOOK</small><strong>${product.name}</strong><em>${product.price.toLocaleString('ko-KR')}원</em></span><b aria-hidden="true">→</b></a></article>`; }).join('')}</div></div>`;
};
